...
export class RouteJsonParser {
  ...
  buildRequestModels(data: RouteJson, modelsRef: Model[]) {
    ...
    if (!endpoint && config.components.routeModel.isEndpointRequired()) {
      ...
      console.log(
        chalk.yellow(
          texts.get("INFO_ROUTE_MODEL_XXX_SKIPPED_ERROR").replace("###", name)
        )
      );
      return;
    }
  ...
  build(
    ...
    for (const data of list) {
      ...
      if (!endpoint && config.components.route.isEndpointRequired()) {
        ...
        console.log(
          chalk.yellow(
            texts.get("INFO_ROUTE_XXX_SKIPPED_ERROR").replace("###", name)
          )
        );
        continue;
      }
      ...
      if (!endpoint && config.components.routeIO.isEndpointRequired()) {
        ...
        console.log(
          chalk.yellow(
            texts.get("INFO_ROUTE_IO_XXX_SKIPPED_ERROR").replace("###", name)
          )
        );
        continue;
      }
